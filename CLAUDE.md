...
Audio Mirror is a Tauri 2 tray app that plays one audio source on N output devices, each with its own volume and mute. The audio engine is a faithful port of OBS Studio's desktop audio capture and audio monitoring; the only intended deviations are 1-to-N monitors, per-output mute, monitors that notice a device that cannot be opened or that goes away while playing, and retry every 3 s (always from the supervisor thread: the WASAPI monitor no longer reopens its device from the audio callback, so one slow device cannot stall the capture and the other outputs), and monitors that follow their device's clock (`audio/drift.rs`). OBS has no such detection: `obs_reset_audio_monitoring` (`libobs/obs.c`) rebuilds monitors only when the user picks another monitoring device, or, on Windows, when the default render device changes; a tray app nobody is watching has to notice on its own. When changing engine behavior, follow the matching OBS code path (the file mapping is in `README.md` and in the module docs of `src-tauri/src/audio/mod.rs`).
...
- `audio/message.rs`: every status message the engine reports. A `Kind` pairs a stable `code` with OBS's English wording; `Message` adds the untranslatable detail (an `HRESULT`, a system error). Nothing matches on message text, and a test checks that every code exists in `ui/i18n.js`.
...
- `audio/swr.rs`: FFI wrapper over libswresample with OBS's settings and mono upmix matrix; each monitor resamples to its device format with `Resampler::adjustable`, whose ratio `set_drift` moves.
- `audio/drift.rs`: `DriftControl`, a PI loop per monitor that holds the level queued ahead of the device (read from the device clock, never from buffer fullness at packet time) by moving the resampler ratio by up to 1000 ppm. Tuned in a simulation in its tests; `wasapi::tests::drift_on_real_devices` (ignored, silent) checks it on the machine's outputs in 150 s.
