...
Audio Mirror is a Tauri 2 tray app that plays one audio source on N output devices, each with its own volume and mute. The audio engine is a faithful port of OBS Studio's desktop audio capture and audio monitoring; the only intended deviations are 1-to-N monitors, per-output mute, and monitors that notice a device that cannot be opened or that goes away while playing, and retry every 3 s. OBS has no such detection: `obs_reset_audio_monitoring` (`libobs/obs.c`) rebuilds monitors only when the user picks another monitoring device, or, on Windows, when the default render device changes; a tray app nobody is watching has to notice on its own. When changing engine behavior, follow the matching OBS code path (the file mapping is in `README.md` and in the module docs of `src-tauri/src/audio/mod.rs`).
